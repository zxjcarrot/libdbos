...
result_prefix="$wd/${EXP_PREFIX_NAME:-redis_linux}_database_size"
...
for size in "${database_sizes[@]}"; do
    ...
    result_file_name="${result_prefix}_dbsize_${dbsize_gb}.json"
    ...
    memtier_benchmark --ratio=1:0 -n `expr 3000000 / 1` --key-pattern=P:P -t 1 --pipeline=1000 -c 1 --key-minimum=1 --key-maximum=$size --data-size=1000  --print-percentiles 50,95,99,99.9,100 --ratio=1:0 --json-out-file=$result_file_name & # > /dev/null 2>&1      
    #memtier_benchmark --ratio=1:0 -n 500000 -t 4 --key-pattern=P:P --pipeline=1000 -c 20 --key-minimum=1 --key-maximum=$size --data-size=1000  --print-percentiles 50,95,99,99.9,100 --ratio=1:0 --json-out-file=$result_file_name &
    benchmark_pid=$!
    sleep 2
