...
zipf = ["0"]
#zipf = [0]
systems = ["tabby_baseline", 'tabby_baseline_prvi', 'tabby_tlbst', 'tabby_tlbst_striped_locks', "tabby"]
...
for ratio in ratios:
    for ff in zipf:
        for system in systems:
            ...
            for db in dbSize:
                path = os.path.join('data', "{}_ablation_ablation_{}_{}_{}.csv".format(system, db, ratio, ff))
...
def draw(data, xlables, figure_name, title):
    ...
    bars = ax.bar(r, values, color=['#3498db', '#e74c3c', '#2ecc71', '#f1c40f', '#1abc9c'], width=bar_width, 
    ...
    # Customize the plot
    ax.set_ylabel('Throughput/Core', fontsize=12, fontweight='bold')
...
data = {
    'tabby_baseline': alldata['tabby_baseline'][100]['0']['Throughput-per-Core'][0],
    'tabby_baseline_prvi': alldata['tabby_baseline_prvi'][100]['0']['Throughput-per-Core'][0],
    'tabby_tlbst': alldata['tabby_tlbst'][100]['0']['Throughput-per-Core'][0],
    'tabby_tlbst_striped_locks': alldata['tabby_tlbst_striped_locks'][100]['0']['Throughput-per-Core'][0],
    'tabby': alldata['tabby'][100]['0']['Throughput-per-Core'][0]
}
draw(data, ['Baseline', '+Elevated\nPrivilege', '+TLB Shootdown\nElimination', "+Thread-Local\nFreelists", "+No-Zeroing"], 'ablation_study_tabby_om_throughput_per_core', '16GB Database, 8GB Buffer Pool, YCSB-C')

# data = {
#     'tabby_baseline': alldata['tabby_baseline'][100]['0']['Throughput-per-Core'][1],
#     'tabby_baseline_prvi': alldata['tabby_baseline_prvi'][100]['0']['Throughput-per-Core'][1],
#     'tabby': alldata['tabby'][100]['0']['Throughput-per-Core'][1]
# }
# draw(data, ['Baseline', '+Privilege\nEscalation', '+TLB Shootdown\nElimination'], 'ablation_study_tabby_im_throughput_per_core', '4GB Database, 8GB Buffer Pool, YCSB-C')
plt.close()
